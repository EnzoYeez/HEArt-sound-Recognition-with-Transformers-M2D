...
├── LICENSE
├── configs/
│   └── circor_m2d_transformer.yaml
├── tools/
│   ├── train.py
│   ├── eval.py
│   └── extract_m2d.py
├── src/
│   ├── data/             # loaders, site-token builder, segment utils
│   ├── models/           # M2DTransformer, heads, pooling
│   ├── losses/           # focal + smoothing, KL distill
│   ├── utils/            # metrics (W.acc/UAR), logging
│   └── engines/          # train/eval loops
├── data/                 # (gitignored)
...
- Use **stratified splits** consistent with prior work; average results across multiple seeds.  
- Keep **input length** consistent (e.g., 5–10 s windows) and follow the **per-patient decision rule** if applicable (record→patient aggregation).  
- When reporting, include **W.acc** and **UAR**; both are needed for fair comparison on CirCor.

---

## 10) Deliverables (for coursework / project)
- **Code** (this repo).  
- **Report** summarizing method, experiments, and comparison to SOTA.  
- **Appendix** with ablations (losses, heads, masking), curves, and confusion matrices.  
- (Optional) **Audio demos** and **attention visualizations**.

---

## 11) Reference Numbers (SOTA to match)
- **M2D on CirCor**: **W.acc ≈ 0.83**, **UAR ≈ 0.71**; ensembles can further improve UAR (e.g., AST+M2D UAR ≈ 0.73).
